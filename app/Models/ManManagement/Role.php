...
class Role extends Model
{
    ...
    public static function currentRole()
    {
        ...
        if ($current_app && Auth::check()) {
            $roles_on_app = Role::where('app_id', $current_app)->get();

            $keanggotaan = AnggotaTimKerja::where('pegawai_id', Auth::id())->pluck('tim_id')->toArray();

            $tim_roles = $roles_on_app->where('type', 'tim')->whereIn('to_role_id', $keanggotaan)->pluck('roles')->toArray();
            $unit_roles = $roles_on_app->where('type', 'unit')->where('to_role_id', Auth::id())->pluck('roles')->toArray();
