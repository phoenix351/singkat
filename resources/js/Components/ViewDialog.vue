...
function closeDialog() {
  ...
  router.get(route("meeting.index"), {
    preserveState: true,
    preserveScroll: true,
  });
  ...
  <Dialog v-model:open="dialogOpen">
    <DialogContent class="lg:max-w-[800px] sm:max-w-[425px]">
      <DialogHeader>
        ...
        <DialogDescription> Detail meeting yang telah Anda buat. </DialogDescription>
      </DialogHeader>
      <div class="grid gap-4">
        ...
        <div class="flex">
          ...
          <div class="text-sm font-md min-w-full">
            <p v-if="meeting.data.occurrences && meeting.data.occurrences.length > 0">
              {{ formatDate(meeting.data.occurrences[0].start_time) }}
            </p>
            <p v-if="meeting.data.occurrences && meeting.data.occurrences.length > 0">
              s.d.
              {{
                formatTanggal(
                  meeting.data.occurrences[meeting.data.occurrences.length - 1].start_time
                )
              }}
        ...
        <div class="flex">
          ...
          <div class="flex items-center">
            <p class="text-sm font-md mr-2 min-w[150px]">
              {{ isPasswordVisible ? meeting.data.password : "••••••" }}
            </p>
        ...
        <div class="flex">
          <Button @click="copy(meeting.data.join_url)">Copy Invitation Link</Button>
        </div>
      </div>
      <!-- <DialogFooter>
